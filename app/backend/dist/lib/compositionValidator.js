...
exports.CompositionValidator = void 0;
const logger_1 = require("../logger");
class CompositionValidator {
    /**
     * Validate composition quality
     */
    static validate(actions, topic) {
        const issues = [];
        const suggestions = [];
        const metrics = {
        ...
        // Overall score
        const score = Math.round((metrics.gridAlignmentScore * 0.25) +
     ...
     */
    static checkGridAlignment(actions, issues, suggestions) {
        let alignedCount = 0;
     ...
     */
    static checkSectioning(actions, issues, suggestions) {
     ...
     */
    static checkSpacing(actions, issues, suggestions) {
     ...
     */
    static checkDensity(actions, issues, suggestions) {
     ...
     */
    static logReport(report, stepId) {
        logger_1.logger.info(`[composition] Step ${stepId}: ${report.passed ? 'PASSED' : 'NEEDS IMPROVEMENT'} (${report.score}%)`);
        logger_1.logger.info(`[composition]   Grid: ${report.metrics.gridAlignmentScore}%`);
        logger_1.logger.info(`[composition]   Sections: ${report.metrics.sectioningScore}%`);
        logger_1.logger.info(`[composition]   Spacing: ${report.metrics.spacingScore}%`);
        logger_1.logger.info(`[composition]   Density: ${report.metrics.densityScore}%`);
        if (report.issues.length > 0) {
            logger_1.logger.warn(`[composition] Issues:`);
            report.issues.forEach(issue => {
                logger_1.logger.warn(`[composition]   - ${issue}`);
            });
        }
        if (report.suggestions.length > 0) {
            logger_1.logger.info(`[composition] Suggestions:`);
            report.suggestions.forEach(suggestion => {
                logger_1.logger.info(`[composition]   + ${suggestion}`);
            });
        }
    }
}
