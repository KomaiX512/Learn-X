...
exports.snapActionToGrid = snapActionToGrid;
exports.snapAllToGrid = snapAllToGrid;
exports.checkGridAlignment = checkGridAlignment;
 ...
 */
function snapActionToGrid(action) {
    ...
    if ('from' in snapped && Array.isArray(snapped.from)) {
        snapped.from = snapped.from.map((val, idx) => idx < 2 ? snapToGrid(val) : val);
    }
    if ('to' in snapped && Array.isArray(snapped.to)) {
        snapped.to = snapped.to.map((val, idx) => idx < 2 ? snapToGrid(val) : val);
    }
    if ('path' in snapped && Array.isArray(snapped.path)) {
        snapped.path = snapped.path.map((point) => {
    ...
    if ('connectedTo' in snapped && typeof snapped.connectedTo === 'object') {
        const conn = snapped.connectedTo;
        if ('x' in conn)
            conn.x = snapToGrid(conn.x);
        if ('y' in conn)
            conn.y = snapToGrid(conn.y);
    }
    return snapped;
}
/**
 ...
 */
function snapAllToGrid(actions) {
    ...
    actions.forEach((original, idx) => {
        ...
        if ('x' in original && 'x' in snap) {
            totalPositions++;
            if (original.x !== snap.x)
                snappedPositions++;
        }
        if ('y' in original && 'y' in snap) {
            totalPositions++;
            if (original.y !== snap.y)
                snappedPositions++;
        }
    });
    const alignmentPercent = totalPositions > 0
        ...
        : 100;
    logger_1.logger.info(`[gridSnapper] Snapped ${snappedPositions}/${totalPositions} positions to grid`);
    logger_1.logger.info(`[gridSnapper] Grid alignment: ${alignmentPercent}% → 100%`);
    return snapped;
}
/**
 * Check grid alignment percentage (for validation)
 */
function checkGridAlignment(actions) {
    let totalPositions = 0;
