...
exports.COMPLEX_STRUCTURE_TESTS = void 0;
exports.runAllTests = runAllTests;
exports.runComplexStructureTest = runComplexStructureTest;
...
// ============================================================================
async function testSubPlannerDistribution(step, topic) {
    console.log('\n╔════════════════════════════════════════════════════════════════╗');
...
ONLY output the JSON array, no explanations.`;
    ...
    try {
        ...
        const hasCoordinates = specifications.some(s => /\(\d+\.\d+,\s*\d+\.\d+\)/.test(s) || /x:\s*\d+\.\d+/.test(s));
        const hasColors = specifications.some(s => /#[0-9a-fA-F]{6}/.test(s) || /rgba?\(/.test(s));
        ...
        const hasScientificTerms = specifications.some(s => scientificTerms.some(term => s.toLowerCase().includes(term.toLowerCase())));
        const animationTerms = ['animate', 'particle', 'wave', 'orbit', 'moving', 'rotation', 'motion'];
        const hasAnimations = specifications.some(s => animationTerms.some(term => s.toLowerCase().includes(term)));
...
// ============================================================================
async function testVisualExecutor(specification, topic, specIndex, totalSpecs) {
    console.log('\n╔════════════════════════════════════════════════════════════════╗');
...
exports.COMPLEX_STRUCTURE_TESTS = COMPLEX_STRUCTURE_TESTS;
async function runComplexStructureTest(test) {
    console.log('\n' + '█'.repeat(75));
    ...
    const stage2Result = await testVisualExecutor(stage1Result.specifications[0], test.topic, 1, stage1Result.specifications.length);
