...
exports.QualityEnforcer = void 0;
...
class QualityEnforcer {
     ...
     */
    static validateActions(actions, step, topic) {
        const issues = [];
        ...
        const hasHighlighting = actions.some(a => a.op === 'drawCircle' ||
            a.op === 'drawVector');
     ...
     */
    static generateRetryPrompt(originalPrompt, report, topic, step) {
...
${originalPrompt}`;
     ...
     */
    static logReport(report, step, topic) {
        logger_1.logger.info(`[quality] Step ${step.id} (${step.tag}) for "${topic}": ${report.passed ? 'PASSED' : 'FAILED'} (${report.score}%)`);
        if (report.issues.length > 0) {
            logger_1.logger.warn(`[quality] Issues found:`);
            report.issues.forEach(issue => {
                logger_1.logger.warn(`[quality]   - ${issue}`);
            });
        }
        if (report.recommendations.length > 0) {
            logger_1.logger.info(`[quality] Recommendations:`);
            report.recommendations.forEach(rec => {
                logger_1.logger.info(`[quality]   + ${rec}`);
            });
        }
    }
}
