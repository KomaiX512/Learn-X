...
exports.generateNotesForStep = generateNotesForStep;
 ...
 */
async function generateNotesInternal(step, topic, subtopic, temperature = TEMPERATURE) {
    logger_1.logger.info(`[notes] Generating SVG keynote for step ${step.id}`);
    logger_1.logger.info(`[notes] Topic: "${topic}"`);
    logger_1.logger.info(`[notes] Subtopic: "${subtopic}"`);
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
        logger_1.logger.error('[notes] GEMINI_API_KEY not set');
        return null;
    }
    try {
        const startTime = Date.now();
        logger_1.logger.debug(`[notes] Using temperature: ${temperature}`);
...
Generate the complete SVG code now. Start with <?xml version="1.0"?> followed by complete <svg width="1200" height="2000" viewBox="0 0 1200 2000"> tags. Use the FULL vertical space (2000px height) to create comprehensive, vertically-stacked educational content with NO horizontal columns.`;
        logger_1.logger.debug(`[notes] Prompt length: ${fullPrompt.length} chars`);
        // Generate without artificial timeout
        const result = await model.generateContent(fullPrompt);
        // Validate response structure
        if (!result || !result.response) {
            logger_1.logger.error('[notes] No response object from API');
            logger_1.logger.error('[notes] Result structure:', JSON.stringify(result || {}, null, 2).substring(0, 500));
            return null;
        }
        ...
        if (result.response.promptFeedback) {
            ...
            if (feedback.blockReason) {
                logger_1.logger.error(`[notes] ⛔ Prompt BLOCKED by Gemini API: ${feedback.blockReason}`);
                logger_1.logger.error(`[notes] Safety ratings:`, JSON.stringify(feedback.safetyRatings || [], null, 2));
                return null;
            }
        }
        if (!result.response.candidates || result.response.candidates.length === 0) {
            logger_1.logger.error('[notes] No candidates in response');
            logger_1.logger.error('[notes] Response:', JSON.stringify(result.response, null, 2).substring(0, 500));
            return null;
        }
        const candidate = result.response.candidates[0];
        // Log finish reason for diagnostics
        logger_1.logger.debug(`[notes] Finish reason: ${candidate.finishReason}`);
        // Check for safety blocks
        if (candidate.finishReason === 'SAFETY' || candidate.finishReason === 'RECITATION') {
            logger_1.logger.error(`[notes] ⛔ Content BLOCKED: ${candidate.finishReason}`);
            if (candidate.safetyRatings) {
                logger_1.logger.error('[notes] Safety ratings:', JSON.stringify(candidate.safetyRatings, null, 2));
            }
            return null;
        }
        ...
        if (candidate.finishReason === 'MAX_TOKENS' || candidate.finishReason === 'LENGTH') {
            logger_1.logger.warn(`[notes] ⚠️ MAX_TOKENS/LENGTH reached - attempting to use partial SVG content`);
        }
        // Check for other non-STOP finish reasons
        if (candidate.finishReason && candidate.finishReason !== 'STOP') {
            logger_1.logger.warn(`[notes] ⚠️ Unusual finish reason: ${candidate.finishReason}`);
        }
        // Extract SVG code
        let svgCode = '';
        try {
            ...
            logger_1.logger.error('[notes] Failed to extract SVG code:', error.message);
            return null;
        }
        if (!svgCode || svgCode.trim().length === 0) {
            logger_1.logger.error('[notes] ❌ EMPTY SVG CODE EXTRACTED');
            logger_1.logger.error('[notes] Finish reason:', candidate.finishReason);
            logger_1.logger.error('[notes] Has content parts:', candidate?.content?.parts ? 'YES' : 'NO');
            if (candidate?.content?.parts) {
                logger_1.logger.error('[notes] Parts count:', candidate.content.parts.length);
                candidate.content.parts.forEach((part, idx) => {
                    logger_1.logger.error(`[notes] Part ${idx}:`, Object.keys(part).join(', '));
                });
            }
            logger_1.logger.error('[notes] This usually means Gemini refused to generate or hit token limits');
            return null;
        }
        ...
        if (!svgCode.includes('<svg')) {
            logger_1.logger.error('[notes] No <svg> tag found in generated content');
            return null;
        }
        ...
        if (openSvgTags > closeSvgTags) {
            logger_1.logger.warn(`[notes] Auto-repairing: Adding ${openSvgTags - closeSvgTags} closing tag(s)`);
        ...
        if (finalOpenTags !== finalCloseTags) {
            logger_1.logger.error(`[notes] Malformed SVG after repair - Open: ${finalOpenTags}, Close: ${finalCloseTags}`);
            return null;
        }
        ...
        logger_1.logger.info(`[notes] ✅ Generated SVG keynote in ${genTime}s (${svgCode.length} chars)`);
        logger_1.logger.info(`[notes] 📊 Elements: text=${textElements}, rect=${rectElements}, line=${lineElements}, total=${totalElements}`);
        ...
        logger_1.logger.info(`[notes] Debug copy saved: ${debugPath}`);
        return svgCode;
    }
    catch (error) {
        logger_1.logger.error(`[notes] Generation failed: ${error.message}`);
        return null;
    }
}
/**
 ...
 */
function getNotesSubtopic(step, topic) {
    // PRIORITY 1: Use AI-generated notesSubtopic from planner (BEST)
    if (step.notesSubtopic && step.notesSubtopic.trim().length > 0) {
        logger_1.logger.info(`[notes] ✅ Using AI-generated notesSubtopic: "${step.notesSubtopic}"`);
    ...
    logger_1.logger.warn(`[notes] ⚠️ No notesSubtopic from planner, using fallback: "${fallback}"`);
    return fallback;
}
/**
 ...
 */
async function generateNotesForStep(step, topic, subtopicOverride, visualDescription) {
    logger_1.logger.info(`[notes] 🎯 Generating SVG keynote with ${MAX_RETRIES} retry attempts`);
    logger_1.logger.info(`[notes] 📚 Subtopic: "${subtopicOverride || step.notesSubtopic || step.desc}"`);
    logger_1.logger.info(`[notes] 🎬 Visual Desc: "${visualDescription?.substring(0, 80)}..."`);
    let lastError = null;
    let lastQuality = { chars: 0, textElements: 0 };
    const overallStartTime = Date.now();
    // Retry loop
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        logger_1.logger.info(`[notes] 🔄 Attempt ${attempt}/${MAX_RETRIES} for step ${step.id}`);
        const startTime = Date.now();
        try {
            const apiKey = process.env.GEMINI_API_KEY;
            if (!apiKey) {
...
Generate the complete SVG code now. Start with <?xml version="1.0"?> followed by complete <svg width="1200" height="2000" viewBox="0 0 1200 2000"> tags. Use the FULL vertical space (2000px height) to create comprehensive, vertically-stacked educational content with NO horizontal columns.`;
            logger_1.logger.info('[notes] Topic: "' + topic + '"');
            logger_1.logger.info('[notes] Subtopic: "' + actualSubtopic + '"');
            logger_1.logger.debug(`[notes] Temperature: ${TEMPERATURE}`);
            logger_1.logger.debug(`[notes] Prompt length: ${fullPrompt.length} chars`);
            // Generate without timeout - let API complete naturally
            const result = await model.generateContent(fullPrompt);
            const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);
            logger_1.logger.info(`[notes] ⏱️  Generation completed in ${elapsed}s`);
            // Validate response structure
            if (!result || !result.response) {
                logger_1.logger.error('[notes] No response object from API');
                logger_1.logger.error('[notes] Result structure:', JSON.stringify(result || {}, null, 2).substring(0, 500));
            ...
            const meetsMinimumQuality = (hasMinText &&
                hasMinVisuals &&
                hasMinTotal &&
                hasMinLength &&
                hasValidViewBox);
            // COMPLETE LOGGING - Show every validation condition
            logger_1.logger.info(`[notes] 📊 Quality Validation (Attempt ${attempt}):`);
            logger_1.logger.info(`[notes]    - Text elements: ${textCount}/8 ${hasMinText ? '✓' : '✗'}`);
            logger_1.logger.info(`[notes]    - Visual elements: ${visualElements}/5 ${hasMinVisuals ? '✓' : '✗'}`);
            logger_1.logger.info(`[notes]    - Total elements: ${totalElementsCount}/20 ${hasMinTotal ? '✓' : '✗'}`);
            logger_1.logger.info(`[notes]    - Content length: ${svgCode.length}/2000 ${hasMinLength ? '✓' : '✗'}`);
            logger_1.logger.info(`[notes]    - ViewBox present: ${hasValidViewBox ? '✓' : '✗'}`);
            logger_1.logger.info(`[notes]    - Overall: ${meetsMinimumQuality ? '✅ PASS' : '❌ FAIL'}`);
            if (meetsMinimumQuality) {
                const totalTime = ((Date.now() - overallStartTime) / 1000).toFixed(2);
                logger_1.logger.info(`[notes] ✅ SUCCESS on attempt ${attempt}/${MAX_RETRIES} (${totalTime}s total)`);
                return svgCode;
            }
            else {
                logger_1.logger.warn(`[notes] ⚠️ Attempt ${attempt} quality check failed - will retry`);
            ...
            lastError = error;
            logger_1.logger.error(`[notes] ❌ Attempt ${attempt}/${MAX_RETRIES} failed: ${error.message}`);
            if (attempt < MAX_RETRIES) {
                const delay = RETRY_DELAY_BASE * attempt; // 2s, 4s, 6s
                logger_1.logger.info(`[notes] ⏸️  Waiting ${delay}ms before retry...`);
    ...
    const totalTime = ((Date.now() - overallStartTime) / 1000).toFixed(2);
    logger_1.logger.error(`[notes] 💥 All ${MAX_RETRIES} attempts failed after ${totalTime}s`);
    logger_1.logger.error(`[notes] Last error: ${lastError?.message}`);
...
async function generateTranscript(step, topic, visualDescriptions) {
    logger_1.logger.warn('[transcript] Legacy function called, redirecting to generateNotesForStep');
 ...
 */
async function generateNotes(step, topic, subtopic, visualDescription) {
    try {
        return await generateNotesForStep(step, topic, subtopic, visualDescription);
    }
    catch (error) {
        logger_1.logger.error('[generateNotes] Failed:', error);
