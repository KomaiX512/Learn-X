...
exports.generateClarificationNarration = generateClarificationNarration;
 ...
 */
async function generateClarificationNarration(input) {
    logger_1.logger.info(`[clarification-narration] Generating for question: "${input.question.substring(0, 60)}..."`);
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
        logger_1.logger.warn('[clarification-narration] GEMINI_API_KEY not set, using fallback');
...
Generate the clarification narration now. Make it friendly, visual-focused, and directly answer their question. Return ONLY the JSON object.`;
        logger_1.logger.debug(`[clarification-narration] Sending prompt to Gemini (${fullPrompt.length} chars)`);
        const result = await model.generateContent(fullPrompt);
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);
        // Validate response
        if (!result?.response) {
        ...
        try {
            ...
            logger_1.logger.error(`[clarification-narration] JSON parse failed: ${parseError.message}`);
            logger_1.logger.error(`[clarification-narration] Raw response: ${responseText.substring(0, 500)}`);
            throw new Error(`JSON parse failed: ${parseError.message}`);
        }
        // Validate structure
        ...
        logger_1.logger.info(`[clarification-narration] ✅ Generated in ${elapsed}s (${wordCount} words, ${duration}s duration)`);
        return {
        ...
        logger_1.logger.error(`[clarification-narration] Generation failed: ${error.message}`);
        logger_1.logger.warn('[clarification-narration] Using fallback narration');
        return generateFallbackClarificationNarration(input);
    }
}
/**
 ...
 */
function generateFallbackClarificationNarration(input) {
    logger_1.logger.info(`[clarification-narration] Creating fallback for: "${input.question}"`);
...
//# sourceMappingURL=narrationGenerator.js.map