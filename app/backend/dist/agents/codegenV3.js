...
async function codeVisual(specification, topic, apiKey) {
    try {
        ...
        logger_1.logger.error(`[codeVisual] Primary generator failed: ${error.message}`);
        logger_1.logger.info(`[codeVisual] Attempting standard fallback...`);
        try {
            // Fallback to standard implementation
            const fallbackOps = await codeVisualStandard(specification, topic, apiKey);
            logger_1.logger.info(`[codeVisual] ✅ Fallback succeeded with ${fallbackOps.length} operations`);
            return fallbackOps;
        }
        catch (fallbackError) {
            logger_1.logger.error(`[codeVisual] Fallback also failed: ${fallbackError.message}`);
            // Return minimal operations rather than failing completely
            logger_1.logger.warn(`[codeVisual] ⚠️ Using emergency minimal operations`);
...
async function codegenV3(step, topic) {
    ...
    try {
        ...
        logger_1.logger.info(`[codegenV3] 📐 STAGE 1: SubPlanner generating precise specifications...`);
        ...
        if (specifications.length < MINIMUM_SPECS) {
            logger_1.logger.warn(`[codegenV3] ⚠️ LOW SPEC COUNT: ${specifications.length} (minimum: ${MINIMUM_SPECS})`);
            logger_1.logger.warn(`[codegenV3] 🚨 Creating emergency specifications to reach minimum...`);
            ...
            logger_1.logger.info(`[codegenV3] ✅ Emergency specs created: now have ${specifications.length} total specs`);
        }
        const specSuccessRate = (specifications.length / TARGET_SPECS) * 100;
        if (specifications.length < TARGET_SPECS) {
            logger_1.logger.warn(`[codegenV3] ⚠️ PARTIAL PLANNING: ${specifications.length}/${TARGET_SPECS} specs (${specSuccessRate.toFixed(0)}%)`);
        }
        else {
            logger_1.logger.info(`[codegenV3] ✅ STAGE 1 SUCCESS: ${specifications.length} specifications created`);
        }
        specifications.forEach((spec, i) => {
            logger_1.logger.info(`[codegenV3]   Spec ${i + 1}: ${spec.substring(0, 100)}...`);
        });
        // STAGE 2: Execute each specification in PARALLEL for speed
        logger_1.logger.info(`[codegenV3] 🚀 STAGE 2: Executing ${specifications.length} specifications in PARALLEL...`);
        const visualPromises = specifications.map(async (spec, index) => {
            try {
                logger_1.logger.info(`[codegenV3]   Visual ${index + 1}/${specifications.length}: Executing...`);
                const ops = await codeVisual(spec, topic, apiKey);
                logger_1.logger.info(`[codegenV3]   Visual ${index + 1}/${specifications.length}: ✅ ${ops.length} operations`);
                return ops;
            }
            catch (error) {
                logger_1.logger.error(`[codegenV3]   Visual ${index + 1}/${specifications.length}: ❌ Failed`, error);
                return [];
            }
        });
        ...
        logger_1.logger.info(`[codegenV3] ✅ TWO-STAGE PIPELINE COMPLETE`);
        logger_1.logger.info(`[codegenV3] ⏱️  Time: ${genTime}s`);
        logger_1.logger.info(`[codegenV3] 📊 Specifications: ${specifications.length}`);
