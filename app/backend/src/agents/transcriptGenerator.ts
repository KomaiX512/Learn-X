...
export async function generateTranscript(
  ...
  logger.info(`[transcript] Generating for step ${step.id} with ${visualDescriptions.length} visuals`);
  
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    logger.error('[transcript] GEMINI_API_KEY not set');
    return null;
  }
...
Output only the transcript:`;
    ...
    logger.debug(`[transcript] Prompt length: ${prompt.length} chars`);
    
    const generationPromise = model.generateContent(prompt);
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Transcript generation timeout')), GENERATION_TIMEOUT)
    );
    
    const result = await Promise.race([generationPromise, timeoutPromise]) as any;
    
    if (!result?.response?.candidates?.[0]) {
      logger.error('[transcript] No valid response from API');
      return null;
    }
    ...
    if (!transcript || transcript.length === 0) {
      logger.error('[transcript] Empty transcript generated');
      return null;
    }
    
    logger.info(`[transcript] ✅ Generated ${transcript.length} chars`);
    return transcript;
