...
export async function generateKeyNotes(
  topic: string,
  lectureSummary?: string,
  steps?: Array<{ title: string; desc?: string }>,
  stepContents?: StepContent[],
  isPartial?: boolean  // Flag if lecture is not complete yet
...
🎯 GENERATE NOW - COMPREHENSIVE EXAM-READY NOTES:`;
  ...
  try {
    logger.info(`[NotesGenerator] Generating key notes for: ${topic} (Context: ${contextStatus}${isPartial ? ' - PARTIAL' : ''})`);
    if (contextStatus === 'PARTIAL') {
      logger.info(`[NotesGenerator] ⚠️  Working with partial context - will fill in missing critical information`);
    }
    ...
    logger.debug(`[NotesGenerator] Raw response length: ${rawText.length}`);
    ...
    logger.info(`[NotesGenerator] ✅ Generated ${parsed.notes.length} categories`);
    return parsed as NotesResponse;
    
  } catch (error: any) {
    logger.error('[NotesGenerator] Generation failed:', error);
