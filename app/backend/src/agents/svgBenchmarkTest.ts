 ...
 */
export async function stressTestWithRetry(
  topic: string,
  description: string,
  apiKey: string,
  maxRetries: number = 5
): Promise<{ success: boolean; operations: any[]; attempts: number; quality: number }> {
  
  const genAI = new GoogleGenerativeAI(apiKey);
  let bestResult = { operations: [], quality: 0 };
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    logger.info(`[STRESS-TEST] Attempt ${attempt}/${maxRetries} for: ${topic}`);
...
Your output must match the complexity of medical textbook illustrations.`
      ...
      logger.info(`[STRESS-TEST] Attempt ${attempt} - Quality: ${quality}/100`);
      ...
      if (quality >= 75) {
        logger.info(`[STRESS-TEST] ✅ SUCCESS on attempt ${attempt}! Quality: ${quality}`);
        return { success: true, operations, attempts: attempt, quality };
      }
      
      // Otherwise retry with adjusted parameters
      logger.warn(`[STRESS-TEST] Quality ${quality} below threshold, retrying...`);
      
    } catch (error: any) {
      logger.error(`[STRESS-TEST] Attempt ${attempt} error:`, error.message);
      
      // Retry strategy based on error type
      if (error.message?.includes('429') || error.message?.includes('quota')) {
        logger.info(`[STRESS-TEST] Rate limit hit, waiting ${attempt * 2} seconds...`);
  ...
  logger.warn(`[STRESS-TEST] Max retries reached. Best quality: ${bestResult.quality}`);
  return { 
 ...
 */
function createBenchmarkPrompt(topic: string, description: string, attempt: number): string {
 ...
 */
function parseOperations(text: string): any[] {
  try {
    // Standard JSON parse
    ...
    if (operations.length > 0) {
      logger.info(`[PARSE] Recovered ${operations.length} operations via fallback`);
      return operations;
    }
 ...
 */
function assessQuality(operations: any[]): number {
  ...
  operations.forEach((op: any) => {
    if (op.op === 'customPath' && op.path) {
      ...
      // Scoring
      if (complexity >= 15) score += 15;  // Excellent complexity
      else if (complexity >= 10) score += 10;  // Good complexity
      else if (complexity >= 5) score += 5;   // Acceptable
      
      // Bonus for curves
      if (curves.length >= 3) score += 5;
    }
    
    if (op.op === 'drawLabel' && op.text) {
      metrics.labels++;
      if (op.text.length > 10) score += 2;  // Detailed label
    }
  ...
  // Overall scoring
  if (metrics.totalOps >= 40) score += 10;
  if (metrics.complexPaths >= 10) score += 15;
  if (metrics.labels >= 8) score += 10;
  if (metrics.avgPathComplexity >= 10) score += 15;
  if (metrics.colors.size >= 5) score += 5;  // Color variety
  
  // Penalties
  if (metrics.totalOps < 20) score -= 20;
  if (metrics.avgPathComplexity < 5) score -= 10;
  if (metrics.labels < 5) score -= 10;
  
  // Log metrics for debugging
  logger.debug(`[QUALITY] Metrics:`, metrics);
...
export const UnitTests = {
   ...
   */
  async testPathGeneration(apiKey: string): Promise<boolean> {
    const result = await stressTestWithRetry(
      'Cell Mitochondria',
      'Create detailed mitochondria with cristae, matrix, and double membrane',
      apiKey,
      3
    );
    
    const passed = result.quality >= 70 && 
                   result.operations.some((op: any) => 
                     op.op === 'customPath' && 
                     op.path && 
                     op.path.length > 100
                   );
    
    logger.info(`[UNIT-TEST] Path Generation: ${passed ? '✅ PASS' : '❌ FAIL'} (Quality: ${result.quality})`);
    return passed;
  },
   ...
   */
  async testLabelGeneration(apiKey: string): Promise<boolean> {
    const result = await stressTestWithRetry(
      'DNA Structure',
      'Show DNA double helix with base pairs labeled',
      apiKey,
      3
    );
    
    const labels = result.operations.filter((op: any) => op.op === 'drawLabel');
    const passed = labels.length >= 8 && 
                   labels.every((op: any) => op.text && op.text.length > 0);
    
    logger.info(`[UNIT-TEST] Label Generation: ${passed ? '✅ PASS' : '❌ FAIL'} (${labels.length} labels)`);
    return passed;
  },
   ...
   */
  async testComplexityStress(apiKey: string): Promise<boolean> {
    const topics = [
    ...
    logger.info(`[UNIT-TEST] Complexity Stress: ${passed ? '✅ PASS' : '❌ FAIL'} (Avg Quality: ${avgQuality.toFixed(1)})`);
    return passed;
  }
};
 ...
 */
export async function runCompleteBenchmarkTest(apiKey: string): Promise<void> {
  console.log('\n════════════════════════════════════════════════════');
  ...
  const tests = [
    { name: 'Path Generation', fn: UnitTests.testPathGeneration },
    { name: 'Label Generation', fn: UnitTests.testLabelGeneration },
    { name: 'Complexity Stress', fn: UnitTests.testComplexityStress }
  ];
...
export default { stressTestWithRetry, UnitTests, runCompleteBenchmarkTest };
