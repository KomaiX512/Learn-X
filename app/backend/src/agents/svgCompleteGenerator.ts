 ...
 */
export function createCompleteSVGPrompt(topic: string, description: string, verticalSection: number): string {
 ...
 */
export function validateCompleteSVG(svgCode: string): {
  valid: boolean;
  score: number;
  issues: string[];
} {
  ...
  // Check for structure
  if (svgCode.includes('<defs>')) score += 10;
  if (svgCode.includes('<style>')) score += 10;
  if (svgCode.includes('viewBox')) score += 10;
...
export async function generateCompleteSVG(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    logger.info(`[SVG-COMPLETE] Attempt ${attempt}/${maxRetries} for section ${verticalSection}`);
    ...
    for (let retryCount = 0; retryCount < 3; retryCount++) {
      try {
        if (retryCount > 0) {
          ...
          logger.info(`[SVG-COMPLETE] Retry ${retryCount}/2 after ${delayMs}ms delay`);
        ...
        if (error.message?.includes('429') || error.message?.includes('rate limit')) {
          logger.warn(`[SVG-COMPLETE] Rate limit hit, will retry...`);
          continue;
        }
    ...
    try {
      if (!result || !result.response) {
        logger.error(`[SVG-COMPLETE] No response from Gemini API`);
        throw new Error('No response from API');
      }

      if (result.response.promptFeedback?.blockReason) {
        logger.error(`[SVG-COMPLETE] Content blocked: ${result.response.promptFeedback.blockReason}`);
        throw new Error(`Content blocked: ${result.response.promptFeedback.blockReason}`);
      }

      // Check for candidates
      if (!result.response.candidates || result.response.candidates.length === 0) {
        logger.error(`[SVG-COMPLETE] No candidates in response`);
        logger.error(`[SVG-COMPLETE] Response object:`, JSON.stringify(result.response, null, 2).substring(0, 500));
        throw new Error('No candidates in API response');
      }

      // Log finish reason for debugging
      const finishReason = result.response.candidates[0]?.finishReason;
      logger.info(`[SVG-COMPLETE] Finish reason: ${finishReason}`);

      let svgCode = result.response.text();

      logger.info(`[SVG-COMPLETE] Received ${svgCode.length} chars`);
      ...
      logger.info(`[SVG-COMPLETE] Raw output (first 500 chars): ${svgCode.substring(0, 500)}`);

      // Extract SVG if wrapped in other content - IMPROVED REGEX
      const svgMatch = svgCode.match(/<\?xml[\s\S]*?<svg[\s\S]*?<\/svg>/i);
      if (svgMatch) {
        svgCode = svgMatch[0];
        logger.info(`[SVG-COMPLETE] Extracted SVG with <?xml header`);
      } else {
        ...
        if (svgMatch2) {
          ...
          logger.info(`[SVG-COMPLETE] Extracted SVG without <?xml header, added it`);
        } else {
          logger.error(`[SVG-COMPLETE] Could not find valid SVG structure in output`);
          logger.error(`[SVG-COMPLETE] Output was: ${svgCode.substring(0, 1000)}`);
        }
      }
      ...
      logger.info(`[SVG-COMPLETE] Quality Score: ${validation.score}/100`);
      if (validation.issues.length > 0) {
        logger.warn(`[SVG-COMPLETE] Issues: ${validation.issues.join(', ')}`);
      }

      // ACCEPT if score >= 50 (reasonable threshold) OR if it has valid SVG structure
      if (validation.score >= 50) {
        logger.info(`[SVG-COMPLETE] ✅ Quality acceptable! Score: ${validation.score}`);
        return svgCode;
      }
      ...
      if (attempt === maxRetries) {
        if (svgCode.includes('<svg') && svgCode.includes('</svg>')) {
          logger.warn(`[SVG-COMPLETE] ⚠️ Last attempt - returning SVG despite score ${validation.score}`);
          return svgCode;
        }
      }
      ...
      if (attempt < maxRetries) {
        logger.warn(`[SVG-COMPLETE] Score too low (${validation.score}), retrying...`);
        continue;
      }
      ...
      logger.error(`[SVG-COMPLETE] Attempt ${attempt} failed: ${error.message}`);

      // Handle rate limiting
      if (error.message?.includes('rate limit') || error.message?.includes('503')) {
        const backoffTime = Math.min(1000 * Math.pow(2, attempt), 10000);
        logger.warn(`[SVG-COMPLETE] Rate limited. Waiting ${backoffTime}ms...`);
 ...
 */
export async function generateCompleteSVGsForStep(
  topic: string,
  descriptions: string[],
  apiKey: string
): Promise<string[]> {
  ...
  for (let i = 0; i < descriptions.length; i++) {
    logger.info(`[SVG-COMPLETE] Generating SVG ${i + 1}/${descriptions.length}`);

    try {
      const svgCode = await generateCompleteSVG(topic, descriptions[i], i, apiKey);
      svgCodes.push(svgCode);
    } catch (error: any) {
      logger.error(`[SVG-COMPLETE] Failed SVG ${i + 1}: ${error.message}`);
