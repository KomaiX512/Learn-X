 ...
 */
export function createInsaneQualityPrompt(topic: string, description: string): string {
 ...
 */
export function validateQuality(operations: Action[]): {
  valid: boolean;
  score: number;
  issues: string[];
} {
  ...
  if (operations.length >= 60) score += 10; // Bonus for richness
  if (customPathCount >= 10) score += 25;  // Many complex structures
  if (avgPathComplexity >= 8) score += 30;  // Rich Bezier curves
  if (labelCount >= 10) score += 15;  // Comprehensive labeling
  if (animationCount >= 3) score += 5;  // Dynamic elements
  
  // Issues
  if (operations.length < 30) issues.push('Need 40-80 operations for richness');
  if (customPathCount < 8) issues.push('Need more complex SVG paths');
  if (avgPathComplexity < 6 && customPathCount > 0) issues.push('Paths need more curves/commands');
  if (labelCount < 8) issues.push('Need comprehensive labeling');
 ...
 */
export async function generateInsaneVisuals(
  topic: string,
  description: string,
  apiKey: string,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    logger.info(`[SVG-MASTER] Attempt ${attempt}/${maxRetries} for quality generation`);
    ...
    for (let retryCount = 0; retryCount < 3; retryCount++) {
      try {
        if (retryCount > 0) {
          ...
          logger.info(`[SVG-MASTER] Retry ${retryCount}/2 after ${delayMs}ms delay`);
        ...
        if (error.message?.includes('429') || error.message?.includes('rate limit')) {
          logger.warn(`[SVG-MASTER] Rate limit hit, will retry...`);
          continue;
        }
    ...
    try {
      ...
      if (!result || !result.response) {
        logger.error(`[SVG-MASTER] No response object from Gemini API`);
        logger.error(`[SVG-MASTER] Result:`, JSON.stringify(result, null, 2).substring(0, 500));
        throw new Error('No response object from API');
      }
      
      // Check for blocked or safety issues
      if (result.response.promptFeedback?.blockReason) {
        logger.error(`[SVG-MASTER] Content blocked: ${result.response.promptFeedback.blockReason}`);
      ...
      logger.info(`[SVG-MASTER] Raw response length: ${text.length} chars`);
      
      // CRITICAL: Handle empty or too-short responses
      if (!text || text.trim().length === 0) {
        logger.error(`[SVG-MASTER] Empty response from Gemini API`);
        logger.error(`[SVG-MASTER] Response object:`, JSON.stringify(result.response, null, 2).substring(0, 500));
        throw new Error('Empty LLM response');
      }
      
      if (text.length < 50) {
        logger.error(`[SVG-MASTER] Response too short (${text.length} chars): ${text}`);
        throw new Error('Response too short to be valid JSON array');
      }
      
      logger.debug(`[SVG-MASTER] First 500 chars: ${text.substring(0, 500)}`);
      ...
      try {
        ...
        logger.info(`[SVG-MASTER] ✅ Strategy 1: Parsed ${operations.length} operations`);
      } catch (e1) {
        logger.warn(`[SVG-MASTER] Strategy 1 failed: ${e1.message}`);
        ...
        try {
          ...
          if (match) {
            ...
            logger.info(`[SVG-MASTER] ✅ Strategy 2: Parsed ${operations.length} operations`);
          } else {
            throw new Error('No JSON array found');
          }
        } catch (e2) {
          logger.warn(`[SVG-MASTER] Strategy 2 failed: ${e2.message}`);
          ...
          if (operations.length > 0) {
            logger.info(`[SVG-MASTER] ✅ Strategy 3: Extracted ${operations.length} operations`);
          } else {
      ...
      operations = operations.map(op => {
        ...
        // Clamp x1, y1, x2, y2
        if (typeof clamped.x1 === 'number') clamped.x1 = Math.max(0, Math.min(1, clamped.x1));
        if (typeof clamped.y1 === 'number') clamped.y1 = Math.max(0, Math.min(1, clamped.y1));
        if (typeof clamped.x2 === 'number') clamped.x2 = Math.max(0, Math.min(1, clamped.x2));
        if (typeof clamped.y2 === 'number') clamped.y2 = Math.max(0, Math.min(1, clamped.y2));
      ...
      logger.info(`[SVG-MASTER] ✅ Coordinates clamped to 0-1 range`);
      
      // Validate quality
      const validation = validateQuality(operations);
      
      logger.info(`[SVG-MASTER] Quality Score: ${validation.score}/100`);
      
      // Accept if quality score is good OR has minimum rich operations
      if (validation.score >= 50 || operations.length >= 30) {
        logger.info(`[SVG-MASTER] ✅ Quality acceptable! Score: ${validation.score}, Ops: ${operations.length}`);
        return operations;
      } else {
        logger.warn(`[SVG-MASTER] Low quality (${validation.score}): ${validation.issues.join(', ')}`);
        if (attempt < maxRetries) {
          logger.info(`[SVG-MASTER] Retrying...`);
          continue;
        }
      }
      ...
      if (operations.length >= 20) {
        logger.warn(`[SVG-MASTER] ⚠️ Returning ${operations.length} operations despite low score`);
        return operations;
      }
      ...
      logger.error(`[SVG-MASTER] Attempt ${attempt} failed: ${error.message}`);
      ...
      if (error.message?.includes('rate limit') || error.message?.includes('overloaded') || error.message?.includes('503')) {
        ...
        logger.warn(`[SVG-MASTER] Rate limited. Waiting ${backoffTime}ms before retry...`);
 ...
 */
export const TopicSpecializations = {
  biology: (topic: string) => `
