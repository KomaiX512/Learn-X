...
export function validateQuality(operations: Action[]): {
  ...
  // Scoring
  if (operations.length >= 50) score += 20;
  if (customPathCount >= 15) score += 20;
  if (avgPathComplexity >= 8) score += 30;
  if (labelCount >= 10) score += 15;
  if (animationCount >= 5) score += 15;
  
  // Issues
  if (operations.length < 30) issues.push('Too few operations (need 50+)');
  if (customPathCount < 10) issues.push('Too few complex structures');
  if (avgPathComplexity < 5) issues.push('Paths too simple (need more curves/commands)');
  if (labelCount < 8) issues.push('Insufficient labeling');
  if (animationCount < 3) issues.push('Lacks dynamic elements');
