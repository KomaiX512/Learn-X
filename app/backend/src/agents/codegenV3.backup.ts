...
import { generateAnimationsForStep } from './svgAnimationGenerator';
...
No markdown, no explanations, just the JSON array.`;
  ...
  try {
    ...
    logger.info(`[planVisuals] LLM OUTPUT:\n${text.substring(0, 500)}...`);
    ...
    try {
      ...
      logger.info(`[planVisuals] ✅ Strategy 1 success: ${parsed.length} visuals`);
      return parsed;
    } catch {}
    ...
    try {
      ...
      if (match) {
        ...
        parsed = JSON.parse(cleaned);
        logger.info(`[planVisuals] ✅ Strategy 2 success: ${parsed.length} visuals`);
        return parsed;
      }
    } catch {}
    ...
    try {
      ...
      if (parsed.length > 0) {
        logger.info(`[planVisuals] ✅ Strategy 3 success: ${parsed.length} visuals`);
        return parsed;
      }
    } catch {}
    
    // All strategies failed - Use LLM to fix the JSON
    logger.error(`[planVisuals] All JSON strategies failed, using LLM debugger`);
...
Remove markdown, explanations, extra text.`;
    ...
    try {
      ...
      if (match) {
        ...
        logger.info(`[planVisuals] ✅ LLM DEBUGGER SUCCESS: ${parsed.length} visuals`);
        return parsed;
      }
    } catch (e) {
      logger.error(`[planVisuals] LLM debugger failed:`, e);
    }
    
    // Last resort - ultra simple
    logger.error(`[planVisuals] Trying ultra-simple retry`);
...
Output: ["visual 1", "visual 2", "visual 3"]`;
    ...
    try {
      ...
      if (match) {
        const parsed = JSON.parse(match[0]);
        logger.info(`[planVisuals] ✅ SIMPLE RETRY SUCCESS: ${parsed.length} visuals`);
        return parsed;
      }
    } catch {}
    
    // FINAL RESORT: Use syntax recovery agent
    logger.error(`[planVisuals] Trying SYNTAX RECOVERY AGENT as final resort`);
    try {
      const recovered = await recoverJSON(text, 'descriptions');
      if (Array.isArray(recovered) && recovered.length > 0) {
        logger.info(`[planVisuals] ✅ SYNTAX RECOVERY SUCCESS: ${recovered.length} visuals`);
        return recovered;
      }
    } catch (e) {
      logger.error(`[planVisuals] Syntax recovery agent failed:`, e);
    }
    
    // Complete failure
    logger.error(`[planVisuals] COMPLETE FAILURE - all recovery failed including syntax agent`);
    return [];
    
  } catch (e) {
    logger.error(`[planVisuals] Exception:`, e);
    return [];
  }
}
 ...
 */
export async function planVisualsEnhanced(step: PlanStep, topic: string, apiKey: string): Promise<VisualSpec[]> {
  const genAI = new GoogleGenerativeAI(apiKey);
  const model = genAI.getGenerativeModel({ 
    model: PRIMARY_MODEL,
    generationConfig: { 
...
Generate ONLY the JSON array (no markdown, no explanations):`;
  ...
  for (let retryCount = 0; retryCount < 3; retryCount++) {
    try {
      if (retryCount > 0) {
        ...
        logger.info(`[planVisualsEnhanced] Retry ${retryCount}/2 after ${delayMs}ms delay`);
      ...
      if (error.message?.includes('429') || error.message?.includes('rate limit')) {
        logger.warn(`[planVisualsEnhanced] Rate limit hit, will retry...`);
        continue;
      }
  ...
  try {
    const text = result.response.text();
    logger.info(`[planVisualsEnhanced] Received response`);
    ...
    try {
      ...
      logger.info(`[planVisualsEnhanced] ✅ Parsed ${specs.length} specs (${specs.filter(s => s.type === 'animation').length} animations)`);
    } catch {
      const match = text.match(/\[\s*\{[\s\S]*\}\s*\]/);
      if (match) {
        specs = JSON.parse(match[0]);
        logger.info(`[planVisualsEnhanced] ✅ Extracted ${specs.length} specs`);
      }
    }
    
    // Validate spec count and animations
    if (specs.length < 7) {
      logger.warn(`[planVisualsEnhanced] Only ${specs.length} specs, expected 7-9`);
    }
    
    const animCount = specs.filter(s => s.type === 'animation').length;
    if (animCount < 2 && specs.length >= 5) {
      logger.warn(`[planVisualsEnhanced] Only ${animCount} animations, converting some static to animations`);
    ...
    logger.error(`[planVisualsEnhanced] Failed: ${error.message}`);
    return [];
  }
}
...
CRITICAL: Output ONLY the JSON array above. NO text, NO descriptions, ONLY JSON.`;
  ...
  try {
    ...
    try {
      ...
      logger.info(`[codeVisual] ✅ Strategy 1: ${operations.length} operations`);
      const uniqueOps = [...new Set(operations.map((op: any) => op.op))];
      logger.info(`[codeVisual] Operations: ${uniqueOps.join(', ')}`);
      return operations;
    } catch {}
    ...
    try {
      ...
      }).filter(op => op !== null) as Action[];
      
      if (operations.length > 0) {
        logger.info(`[codeVisual] ✅ Strategy 2: ${operations.length} operations`);
        return operations;
      }
    } catch {}
    
    // All strategies failed - Use LLM to fix the JSON
    logger.error(`[codeVisual] All JSON strategies failed, using LLM debugger`);
...
Remove any markdown, explanations, or extra text.`;
    ...
    try {
      ...
      if (match) {
        ...
        logger.info(`[codeVisual] ✅ LLM DEBUGGER SUCCESS: ${ops.length} operations`);
        return ops;
      }
    } catch (e) {
      logger.error(`[codeVisual] LLM debugger failed:`, e);
    }
...
ONLY JSON array.`;
    ...
    try {
      ...
      if (match) {
        const ops = JSON.parse(match[0]);
        logger.info(`[codeVisual] ✅ SIMPLE RETRY SUCCESS: ${ops.length} operations`);
        return validateOperations(ops);
      }
    } catch {}
    
    // FINAL RESORT: Use syntax recovery agent
    logger.error(`[codeVisual] Trying SYNTAX RECOVERY AGENT as final resort`);
    try {
      const recovered = await recoverJSON(text, 'operations');
      if (Array.isArray(recovered) && recovered.length > 0) {
        const validated = validateOperations(recovered);
        logger.info(`[codeVisual] ✅ SYNTAX RECOVERY SUCCESS: ${validated.length} valid operations`);
        return validated;
      }
    } catch (e) {
      logger.error(`[codeVisual] Syntax recovery agent failed:`, e);
    }
    
    // Complete failure
    logger.error(`[codeVisual] COMPLETE FAILURE - all recovery attempts failed including syntax agent`);
    return [];
    
  } catch (e) {
    logger.error(`[codeVisual] Exception:`, e);
    return [];
  }
}
...
- Include particles/waves for dynamic effects`;
  ...
  try {
    ...
    try {
      ...
      if (match) {
        const parsed = JSON.parse(match[0]);
        logger.info(`[codegenV3-FAST] ✅ Extracted: ${parsed.length} operations`);
        return validateOperations(parsed);
      }
    } catch {}
    
    logger.warn(`[codegenV3-FAST] Using syntax recovery...`);
    const recovered = await recoverJSON(text, 'operations');
    return validateOperations(recovered);
    
  } catch (error) {
    logger.error(`[codegenV3-FAST] ❌ Failed:`, error);
    return [];
  }
}
...
export async function codegenV3(step: PlanStep, topic: string): Promise<CodegenChunk | null> {
  ...
  if (!apiKey) {
    logger.error('[codegenV3] GEMINI_API_KEY not set');
    return null;
  }
  ...
  try {
    ...
    logger.info(`[codegenV3] 📐 STAGE 1: Enhanced SubPlanner generating specifications (static + animation)...`);
    ...
    logger.info(`[codegenV3] ✅ STAGE 1 SUCCESS: ${visualSpecs.length} specs (${staticSpecs.length} static, ${animationSpecs.length} animations)`);
    visualSpecs.forEach((spec, i) => {
      const prefix = spec.type === 'animation' ? `🎬 [${spec.animationType}]` : '📐 [static]';
      logger.info(`[codegenV3]   ${prefix} Spec ${i+1}: ${spec.description.substring(0, 80)}...`);
    });
    
    // STAGE 2: Execute specifications with routing to appropriate generators
    logger.info(`[codegenV3] 🚀 STAGE 2: Executing ${visualSpecs.length} specifications (PARALLEL)...`);
    
    const visualPromises = visualSpecs.map(async (spec, index) => {
      try {
        if (spec.type === 'animation') {
          logger.info(`[codegenV3]   🎬 Animation ${index+1}: Generating ${spec.animationType} animation...`);
          const svgCode = await import('./svgAnimationGenerator').then(m => 
            m.generateSVGAnimation(topic, spec.description, spec.animationType || 'flow', apiKey)
          );
          const action: Action = {
            op: 'customSVG',
            svgCode,
          ...
          logger.info(`[codegenV3]   Animation ${index+1}: ✅ Generated SVG animation`);
          return [action];
        } else {
          logger.info(`[codegenV3]   📐 Static ${index+1}: Generating static visual...`);
          const ops = await codeVisual(spec.description, topic, apiKey);
          logger.info(`[codegenV3]   Static ${index+1}: ✅ ${ops.length} operations`);
          return ops;
        }
      } catch (error: any) {
        logger.error(`[codegenV3]   Visual ${index+1}: ❌ Failed - ${error.message}`);
        return [];
      }
    });
    ...
    logger.info(`[codegenV3] ✅ TWO-STAGE PIPELINE COMPLETE (WITH ANIMATIONS)`);
    logger.info(`[codegenV3] ⏱️  Time: ${genTime}s`);
    logger.info(`[codegenV3] 📊 Specifications: ${visualSpecs.length} (${staticSpecs.length} static, ${animationSpecs.length} animations)`);
