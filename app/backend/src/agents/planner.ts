...
export async function plannerAgent(query: string, difficulty: 'easy' | 'medium' | 'hard' = 'hard'): Promise<Plan> {
  ...
  logger.info(`[Planner] Difficulty: ${difficulty}, Steps: ${stepCount}`);
...
Topic: ${query}`;
  ...
  logger.info(`[Planner] ✅ Generated ${plan.steps.length} step(s) for difficulty: ${difficulty}`);
  return plan;
}
