...
export async function codegenV3(step: PlanStep, topic: string): Promise<CodegenChunk | null> {
  logger.info(`[codegenV3] Generating step ${step.id}: ${step.desc}`);
  ...
  try {
    ...
    if (!result || !result.response) {
      logger.error('[codegenV3] No response object from API');
      throw new Error('No response object from API');
    }
    
    // Check for candidates (API might block content)
    if (!result.response.candidates || result.response.candidates.length === 0) {
      logger.error('[codegenV3] No candidates in response');
      logger.error('[codegenV3] Response:', JSON.stringify(result.response, null, 2).substring(0, 500));
    ...
    if (candidate.finishReason) {
      logger.debug(`[codegenV3] Finish reason: ${candidate.finishReason}`);
      
      // BLOCK ONLY TRUE SAFETY ISSUES
      if (candidate.finishReason === 'SAFETY' || candidate.finishReason === 'RECITATION') {
        logger.error('[codegenV3] Content blocked by safety filters!');
        logger.error('[codegenV3] Safety ratings:', JSON.stringify(candidate.safetyRatings));
    ...
    try {
      ...
      logger.error('[codegenV3] Failed to extract text:', error.message);
      
      // Final attempt: log structure and try to get any text
      logger.error('[codegenV3] Candidate structure:', JSON.stringify({
    ...
    if (!svgCode || svgCode.trim().length === 0) {
      logger.error('[codegenV3] ❌ NO TEXT EXTRACTED from response');
      logger.error('[codegenV3] Candidate:', JSON.stringify(candidate, null, 2).substring(0, 1000));
      throw new Error('No text content in API response');
    }
    
    logger.debug(`[codegenV3] Received ${svgCode.length} chars from API`);
    ...
    if (!svgCode.includes('<svg')) {
      logger.error('[codegenV3] ❌ NO SVG OPENING TAG');
      logger.error('[codegenV3] Content received:');
      logger.error(svgCode.substring(0, 1000));
    ...
    logger.debug(`[codegenV3] SVG tag count: ${openSvgTags} open, ${closeSvgTags} close`);
    ...
    if (openSvgTags > closeSvgTags) {
      ...
      logger.warn(`[codegenV3] 🔧 AUTO-REPAIR: Missing ${openSvgTags - closeSvgTags} closing tag(s) (finishReason: ${candidate?.finishReason})`);
    ...
    if (finalOpenTags !== finalCloseTags) {
      logger.error(`[codegenV3] ❌ MALFORMED SVG after repair - Open: ${finalOpenTags}, Close: ${finalCloseTags}`);
      logger.error('[codegenV3] Content preview:');
      logger.error(svgCode.substring(0, 500) + '\n...\n' + svgCode.substring(Math.max(0, svgCode.length - 500)));
    ...
    if (distanceFromEnd > 50) {
      logger.warn(`[codegenV3] ⚠️ ${distanceFromEnd} chars after final </svg> (possible junk content)`);
    }
    ...
    logger.info(`[codegenV3] ✅ Generated SVG in ${genTime}s (${svgCode.length} chars)`);
    logger.info(`[codegenV3] 🎬 ANIMATIONS: ${totalAnimations} total (<animate>: ${animateCount}, <animateMotion>: ${animateMotionCount}, <animateTransform>: ${animateTransformCount})`);
    logger.info(`[codegenV3] 📊 CONTENT: labels=${textLabels}, shapes=${shapes}`);
    
    // DIAGNOSTIC: Save SVG to file for inspection
    if (totalAnimations === 0) {
      logger.error(`[codegenV3] ❌ ZERO ANIMATIONS GENERATED! Saving output for debugging...`);
      const fs = require('fs');
      const debugPath = `/tmp/debug-step${step.id}-NO-ANIMATIONS.svg`;
      fs.writeFileSync(debugPath, svgCode);
      logger.error(`[codegenV3] Saved to: ${debugPath}`);
    } else {
      logger.info(`[codegenV3] ✅ ANIMATIONS PRESENT - Quality check passed`);
      // Save first successful one for reference
      if (step.id === 1) {
        const fs = require('fs');
        const debugPath = `/tmp/debug-step${step.id}-WITH-ANIMATIONS.svg`;
        fs.writeFileSync(debugPath, svgCode);
        logger.info(`[codegenV3] Reference saved to: ${debugPath}`);
      }
    }
