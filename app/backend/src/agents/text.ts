...
export async function textAgent(
  step: PlanStep,
  topic: string
): Promise<TextChunk> {
  logger.debug(`[text] Generating text for step ${step.id}: ${step.desc}`);
...
OUTPUT ONLY THE JSON ARRAY:`;
  ...
  try {
    ...
    const chunk = JSON.parse(jsonText) as TextChunk;
    ...
    logger.debug(`[text] Generated ${chunk.actions.length} text labels for step ${step.id}`);
    return chunk;
