...
export class PerformanceMonitor extends EventEmitter {
  ...
  private activeRequests: Map<string, RequestTracker>;
  private requestHistory: RequestTracker[];
  private readonly MAX_HISTORY = 100;
  private readonly METRICS_INTERVAL = 60000; // Update metrics every minute
  ...
  startRequest(sessionId: string): void {
    ...
    logger.debug(`[monitor] Started tracking request ${sessionId}`);
  }
  ...
  endPlanGeneration(sessionId: string, success: boolean = true): void {
    const tracker = this.activeRequests.get(sessionId);
    if (tracker && tracker.planStartTime) {
      ...
      logger.debug(`[monitor] Plan generation for ${sessionId} took ${duration}ms`);
    }
  }
  ...
  endStepGeneration(sessionId: string, stepId: number, success: boolean = true): void {
    const tracker = this.activeRequests.get(sessionId);
    if (tracker) {
      ...
      if (startTime) {
        ...
        logger.debug(`[monitor] Step ${stepId} generation for ${sessionId} took ${duration}ms`);
      }
    }
  }
  ...
  completeRequest(sessionId: string, success: boolean = true): void {
    const tracker = this.activeRequests.get(sessionId);
    if (tracker) {
      ...
      logger.debug(`[monitor] Completed request ${sessionId} in ${totalTime}ms (success: ${success})`);
