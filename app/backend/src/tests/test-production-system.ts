...
import { recoverJSON, validateOperations } from '../agents/syntaxRecoveryAgent';
...
async function testSyntaxRecovery() {
  ...
  try {
    const recovered = await recoverJSON(malformedDescriptions, 'descriptions');
  ...
  try {
    const recovered = await recoverJSON(malformedOperations, 'operations');
    const validated = validateOperations(recovered);
    console.log('✅ PASS: Recovered operations:', validated.length);
    console.log('   Operations:', validated.map((op: any) => op.op).join(', '));
  } catch (error) {
...
function testOperationValidation() {
  ...
  const validated = validateOperations(testOps);
  
  console.log(`Input operations: ${testOps.length}`);
  console.log(`Valid operations: ${validated.length}`);
  console.log('Valid ops:', validated.map((op: any) => op.op).join(', '));
...
async function testRetryStrategy() {
  ...
  try {
    ...
    const result = await codegenV3WithRetry(testStep, topic);
    ...
    if (result && result.actions && result.actions.length >= 15) {
      ...
      const opTypes = new Set(result.actions.map((a: any) => a.op));
...
async function testE2EGeneration() {
  ...
  const testCases = [
    {
      ...
      step: { id: 1, desc: 'Visualize sine wave and basic trigonometric relationships', compiler: 'js' as const, complexity: 2, tag: 'hook' }
    },
    {
      topic: 'Neural Network Architecture',
      step: { id: 2, desc: 'Show neural network layers and connections', compiler: 'js' as const, complexity: 3, tag: 'intuition' }
    }
  ];
  ...
  for (const testCase of testCases) {
    ...
    try {
      console.time(`  Generation time`);
      const result = await codegenV3(testCase.step, testCase.topic);
      console.timeEnd(`  Generation time`);
      ...
      if (result && result.actions) {
        ...
        const hasCustomPath = result.actions.some((a: any) => a.op === 'customPath');
        const hasLabel = result.actions.some((a: any) => a.op === 'drawLabel');
        
        if (hasLabel) console.log(`  ✅ Has labels (contextual)`);
        if (hasCustomPath) console.log(`  ✅ Has custom shapes (rich)`);
...
function testNullHandling() {
  ...
  const result1 = validateOperations(null as any);
  ...
  const result2 = validateOperations(undefined as any);
  ...
  const result3 = validateOperations(malformed);
