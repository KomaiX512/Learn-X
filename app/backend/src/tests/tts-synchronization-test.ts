...
import { generateStepNarrationWithAudio, getInterVisualDelay } from '../services/audio-narration-service';
import { PlanStep } from '../types';
import { logger } from '../logger';

interface VisualPlaybackSimulation {
  visualNumber: number;
  animationDuration: number; // ms
  narrationDuration: number; // ms (from TTS)
  interVisualDelay: number; // ms
 ...
 */
async function simulateVisualPlayback(
  visualNumber: number,
  animationDuration: number,
  narrationAudio: { audioContent: Buffer; audioDuration: number } | null,
  interVisualDelay: number
): Promise<VisualPlaybackSimulation> {
...
async function testSynchronizationTiming() {
  ...
  const case2 = await simulateVisualPlayback(
    2,
    2000, // animation: 2s
  ...
  const case3 = await simulateVisualPlayback(
    3,
    4000, // animation: 4s
...
async function testNoEarlyTransitions() {
  ...
  const visual1Promise = simulateVisualPlayback(
    1,
    3000, // animation: 3s
...
async function testRealTTSIntegration() {
  ...
  try {
    ...
    const narrationResult = await generateStepNarrationWithAudio(
      step,
      topic,
      visuals,
      'test-session-123'
    ...
    for (let i = 0; i < narrationResult.narrations.length; i++) {
      ...
      const result = await simulateVisualPlayback(
        narration.visualNumber,
        animationDuration,
        { audioContent: Buffer.from(''), audioDuration: narration.duration },
        interVisualDelay
      );
