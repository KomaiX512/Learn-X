...
import { generateStepNarration, VisualInput, StepNarration } from '../agents/narrationGenerator';
...
interface VisualGenerationResult {
  ...
  notesSvgCode: string | null;
  animationActions: any[];
  animationMetadata: Array<{ index: number; actionCount: number }>;
  visualDescriptions: Array<{ visualNumber: number; description: string }>;
}
 ...
 */
async function simulateStepVisualsGeneration(
  step: PlanStep,
  topic: string
): Promise<VisualGenerationResult & { narration?: StepNarration }> {
  console.log(`\n${'='.repeat(70)}`);
  ...
  const visualDescriptions: Array<{ visualNumber: number; description: string }> = [];
  ...
  let stepNarration: StepNarration | undefined = undefined;
  ...
  try {
    ...
    if (visualInputs.length > 0) {
      stepNarration = await generateStepNarration(step, topic, visualInputs);
...
async function testMultipleStepsParallel() {
  ...
  try {
    ...
    const allSuccessful = results.every(r => 
      r.animationActions.length > 0 && r.narration !== undefined
    );
