 ...
 */
async function simulateGenerateStepVisuals(
  step: PlanStep,
  topic: string,
  sessionId: string
) {
  ...
  if (ENABLE_NOTES_GENERATION) {
    ...
    const notesPromise = (async () => {
      try {
        const svgCode = await generateNotes(step, topic, step.desc);
        if (!svgCode) {
  ...
  // Generate animations
  const animationMetadata: Array<{ index: number; actionCount: number }> = [];
  ...
  let stepNarration: any = undefined;
  ...
  try {
    ...
    if (visualInputs.length > 0) {
      ...
      console.log('\n📊 Narration Details:');
      stepNarration.narrations.forEach((n: any, idx: number) => {
  ...
  if (notesAction) allActions.push(notesAction);
  ...
  const eventData = {
    ...
    ttsConfig: {
      ...
      interVisualDelay: getInterVisualDelay(),
 ...
 */
function validateEventData(eventData: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  
  console.log('\n📊 Validating Event Data Structure...');
  
  // Check basic structure
  if (!eventData.type) errors.push('Missing type field');
  if (!eventData.stepId) errors.push('Missing stepId field');
  ...
  if (!eventData.narration) {
    ...
    if (!Array.isArray(narration.narrations)) {
      ...
      narration.narrations.forEach((n: any, idx: number) => {
 ...
 */
function validateAudioFiles(sessionId: string, narrations: any[]): boolean {
