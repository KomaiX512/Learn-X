...
import { generateStepNarration, generateSingleNarration, VisualInput } from '../agents/narrationGenerator';
...
<svg width="1200" height="2000" viewBox="0 0 1200 2000">
  ...
  try {
    ...
    const result = await generateSingleNarration(mockStep, mockTopic, visual);
...
async function testBatchGeneration() {
  ...
  try {
    const startTime = Date.now();
    const result = await generateStepNarration(mockStep, mockTopic, visuals);
    const elapsed = Date.now() - startTime;
...
async function testTimeEfficiency() {
  ...
  try {
    ...
    const seqPromises: Promise<any>[] = [];
    
    for (const visual of visuals) {
      seqPromises.push(generateSingleNarration(mockStep, mockTopic, visual));
    }
    ...
    const batchResult = await generateStepNarration(mockStep, mockTopic, visuals);
...
async function testContextualAccuracy() {
  ...
  try {
    const result = await generateStepNarration(
      { ...mockStep, desc: 'Convolutional Neural Networks' },
      mockTopic,
      visuals
    );
...
async function testParallelism() {
  ...
  const visualSets = steps.map((step, idx) => [
    { type: 'notes' as const, visualNumber: 0, svgCode: `<svg>Step ${idx + 1}</svg>`, description: step.desc },
    { type: 'animation' as const, visualNumber: 1, actionCount: 30, description: 'Animation 1' }
  ]);
  ...
  try {
    ...
    const promises = steps.map((step, idx) =>
      generateStepNarration(step, mockTopic, visualSets[idx])
    );
