...
async function analyzePlanGeneration(query: string): Promise<{ plan: any, time: number, success: boolean }> {
  console.log('\n' + '═'.repeat(80));
...
async function analyzeStepGeneration(
  step: PlanStep,
  topic: string,
  stepIndex: number,
  totalSteps: number
...
async function runProductionTest() {
  ...
  const lectureAnalysis: LectureAnalysis = {
    query: TEST_QUERY,
    planGenerated: false,
    planTime: 0,
    totalSteps: 0,
...
function analyzeArchitectureLimitations(analysis: LectureAnalysis) {
  console.log('\n' + '═'.repeat(80));
...
function printFinalAnalysis(analysis: LectureAnalysis) {
  console.log('\n');
  console.log('╔' + '═'.repeat(78) + '╗');
