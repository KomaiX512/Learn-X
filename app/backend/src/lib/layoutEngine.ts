...
export interface LayoutConfig {
  minDistance: number;      // Minimum distance between labels
  anchorStrength: number;    // How strongly labels pull toward anchor
  repulsionStrength: number; // How strongly labels repel each other
 ...
 */
export function calculateTextDimensions(text: string, fontSize: number = 16): { width: number; height: number } {
 ...
 */
export function applyAntiOverlapLayout(
  ...
  for (let iter = 0; iter < fullConfig.iterations; iter++) {
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      positions[i].x = Math.max(
        fullConfig.bounds.minX,
        Math.min(fullConfig.bounds.maxX - positions[i].width, positions[i].x)
      );
      positions[i].y = Math.max(
        fullConfig.bounds.minY,
        Math.min(fullConfig.bounds.maxY - positions[i].height, positions[i].y)
      );
    }
 ...
 */
export function fixLabelOverlap(actions: any[]): any[] {
  // Extract all labels
  ...
  const labels: LabelPosition[] = labelActions.map((action, i) => {
 ...
 */
export const POSITIONING_STRATEGIES = {
  /**
   * Arrange labels in a grid to avoid overlap
   */
  grid(labels: { text: string; fontSize?: number }[], columns: number = 3): { x: number; y: number }[] {
   ...
   */
  circular(count: number, centerX: number = 0.5, centerY: number = 0.5, radius: number = 0.3): { x: number; y: number }[] {
    const positions: { x: number; y: number }[] = [];
   ...
   */
  vertical(count: number, startX: number = 0.1, startY: number = 0.2, spacing: number = 0.08): { x: number; y: number }[] {
    const positions: { x: number; y: number }[] = [];
