 ...
 */
function identifyExpansionOpportunities(actions: Action[]): {
  type: string;
  ...
  const sections = actions.filter(a => 
    a.op === 'drawLabel' && 
    (a as any).text?.match(/^[①②③④⑤]/)
  );
  ...
  sections.forEach((section, idx) => {
    const sectionLabel = (section as any).text || '';
    const sectionY = (section as any).y || 0.5;
    const sectionX = (section as any).x || 0.5;
    ...
    const visuals = nearbyOps.filter(a => 
      !['drawLabel', 'drawTitle', 'delay'].includes(a.op)
    ).length;
    
    const labels = nearbyOps.filter(a => 
      a.op === 'drawLabel' && !(a as any).text?.match(/^[①②③④⑤]/)
    ).length;
 ...
 */
function detectDomain(context: string, allActions: Action[]): string {
  const ctx = context.toLowerCase();
  const allText = allActions
    .filter(a => 'text' in a || 'label' in a)
    .map(a => ((a as any).text || (a as any).label || '').toLowerCase())
    .join(' ');
 ...
 */
function generateComplementaryVisuals(
  opportunities: any[],
  currentOps: number,
  targetOps: number,
  allActions: Action[] = []
): Action[] {
  const additions: Action[] = [];
...
export function expandOperations(
  actions: Action[],
  targetRange: { min: number; max: number } = { min: MIN_OPS, max: MAX_OPS }
): Action[] {
  const currentCount = actions.length;
  
  // Already in target range
  if (currentCount >= targetRange.min && currentCount <= targetRange.max) {
    logger.info(`[expander] Operations already in range: ${currentCount}`);
    return actions;
  }
  
  // Too many operations (trim later if needed)
  if (currentCount > targetRange.max) {
    logger.warn(`[expander] Too many operations: ${currentCount} (max ${targetRange.max})`);
    return actions;
  }
  ...
  logger.info(`[expander] Expanding from ${currentCount} to ~${target} operations (+${needed})`);
  
  // Identify opportunities
  const opportunities = identifyExpansionOpportunities(actions);
  logger.info(`[expander] Found ${opportunities.length} expansion opportunities`);
  
  // Generate additions
  const additions = generateComplementaryVisuals(opportunities, currentCount, target);
  logger.info(`[expander] Generated ${additions.length} additional operations`);
  
  // Merge intelligently
  const expanded = intelligentMerge(actions, additions);
  
  logger.info(`[expander] Final count: ${currentCount} → ${expanded.length} operations`);
 ...
 */
export function needsExpansion(actions: Action[]): boolean {
